...
## Acceptance Criteria
- [x] Undo/redo works across edits made in any of the editors (items 003-008), not just the most recently used one
- [x] Closing the tab or navigating away with unsaved changes triggers a browser confirmation prompt; with no unsaved changes it does not
- [x] The new-character wizard produces a valid minimal character (loadable by the editors) from either a blank template or a chosen preset
- [x] Undo past the last recorded change, or redo past the most recent undo, is a no-op with no error, not a crash or history corruption
