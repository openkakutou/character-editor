...
## Acceptance Criteria
- [x] `appVersion` is derived from `package.json`'s `version` field, not a separately maintained literal
- [x] A test pins that `appVersion` matches `package.json`'s `version` field directly (reading the file independently, not through `appVersion` itself), so a future drift is caught by the test suite
- [x] Existing behavior (the version shown in the toolbar, the document title) is unchanged
